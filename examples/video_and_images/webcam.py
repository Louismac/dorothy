...
class CustomPlayer(AudioDevice):
    ...
    def __init__(self, get_frame: Callable[[int], npt.NDArray[np.float32]], frame_size: int = 512, **kwargs):
        ...
        # DON'T start generation thread in __init__
        self.generate_thread: Optional[threading.Thread] = None
        
    def _ensure_generator_running(self) -> None:
        """Ensure the generator thread is running."""
        if self.generate_thread is None or not self.generate_thread.is_alive():
            self._shutdown_event.clear()
            self.generate_thread = threading.Thread(target=self._fill_buffers, daemon=True)
            self.generate_thread.start()
            
    def _fill_buffers(self) -> None:
        """Fill buffers continuously using custom callback."""
        # Pre-fill all buffers
        try:
            if self.get_frame is not None:
                print("Pre-filling buffers...")
                for i in range(len(self.buffers)):
                    self.buffers[i] = self.get_frame(self.frame_size).astype(np.float32)
                print(f"Buffers filled. Stats: min={self.buffers[0].min():.4f}, max={self.buffers[0].max():.4f}")
        except Exception as e:
            warnings.warn(f"Error pre-filling buffers: {e}")
        
        # Continuously generate into next write buffer
        while not self._shutdown_event.is_set():
            if self.get_frame is not None:
                try:
                    # Calculate next write index
                    with self._buffer_lock:
                        next_write = (self.write_idx + 1) % len(self.buffers)
                        
                        # Don't overwrite buffer being read
                        if next_write == self.read_idx:
                            time.sleep(0.001)
                            continue
                    
                    # Generate buffer outside lock
                    new_buffer = self.get_frame(self.frame_size).astype(np.float32)
                    
                    # Write to buffer
                    with self._buffer_lock:
                        self.buffers[next_write] = new_buffer
                        self.write_idx = next_write
                        
                except Exception as e:
                    warnings.warn(f"Custom callback error: {e}")
                    with self._buffer_lock:
                        self.buffers[next_write].fill(0)
    ...
    def audio_callback(self) -> npt.NDArray[np.float32]:
        ...
        with self._buffer_lock:
            current_buffer = self.buffers[self.read_idx]
        
        # Ensure we don't go out of bounds
        end_idx = min(self.current_sample + self.buffer_size, self.frame_size)
        audio_buffer = current_buffer[self.current_sample:end_idx]
        
        # Pad if we got less than buffer_size
        if len(audio_buffer) < self.buffer_size:
            padding = np.zeros(self.buffer_size - len(audio_buffer), dtype=np.float32)
            audio_buffer = np.concatenate([audio_buffer, padding])
        
        self.current_sample += self.buffer_size
        
        # Switch to next buffer when current is exhausted
        if self.current_sample >= self.frame_size:
            self.current_sample = 0
            with self._buffer_lock:
                # Move to next read buffer
                next_read = (self.read_idx + 1) % len(self.buffers)
                # Don't advance if we'd catch up to write
                if next_read != self.write_idx:
                    self.read_idx = next_read
    ...
    def play(self) -> None:
        ...
        # Give generator time to pre-fill all buffers
        time.sleep(0.1)
        super().play()
...
class Sampler:
        ...
        def get_frame(size: int) -> npt.NDArray[np.float32]:
            ...
            with self._lock:
                for i, p in enumerate(self.sample_pos):
                    if p >= 0 and i < len(self.samples):
                        sample = self.samples[i]
                        end = p + size
                        
                        if end >= len(sample):
                            # Sample ends within this buffer
            ...
            # Clip to prevent distortion from summing
            return np.clip(audio, -1.0, 1.0)
        
        # Start DSP stream with larger frame size for efficiency
        self.audio.start_dsp_stream(get_frame, sr=44100, buffer_size=512, frame_size=2048)
    ...
    def load(self, paths: List[str], sr: int = 44100) -> None:
        """
        ...
        for path in paths:
            try:
                ...
                new_samples.append(y.astype(np.float32))
            except Exception as e:
                warnings.warn(f"Could not load sample {path}: {e}")
                new_samples.append(np.zeros(1024, dtype=np.float32))
